...
else
{
	if (isset($_REQUEST["param"])) {
	    if ($_REQUEST["param"] == "add_new_dynamic_row_for_image") {
			...
			{
				$url = explode("wp-content", $img_path);
				$image_src = GALLERY_BK_THUMB_SMALL_URL . $img_name;
				
				/*
				 * To copy the image from uploads to gallery-bank folder
				 */
				$file_path = GALLERY_BK_THUMB_WP_UPLOADS_URL . $url[1];  
				$file_name_exct = explode("/", $file_path);
	        ...
	        $column2 = "<a href=\"javascript:void(0);\" title=\"" . $videoUrl . "\" >
					...
					<label><strong>Video</strong></label><br/><label>" . date("F j, Y") . "</label>";
	        array_push($dynamicArray, $column2);
	        ...
	        else
	        {
				...
				elseif(preg_match("/flickr\.com(?!.+\/show\/)/i", $ux_img_name))
				{
					$video_thumbnail_path = GALLERY_BK_PLUGIN_URL . "/assets/images/video.jpg"; 
				}
				elseif(preg_match("/tudou\.com/i", $ux_img_name))
				{
					$video_thumbnail_path = GALLERY_BK_PLUGIN_URL . "/assets/images/video.jpg"; 
				}
	            $wpdb->query
